...
from models.model import BotSTHCL  
...
class Trainer:
    def __init__(self, args):
        ...
        self.des_tensor = self.dataset.des_tensor
        self.tweets_tensor = self.dataset.tweets_tensor
        self.num_prop = self.dataset.num_prop
        self.category_prop = self.dataset.category_prop
        self.labels = self.dataset.labels

        
        self.train_right = self.dataset.train_right
        self.train_n_id = self.dataset.train_n_id
        self.train_edge_index = self.dataset.train_edge_index
        self.train_edge_type = self.dataset.train_edge_type
        self.train_exist_nodes = self.dataset.train_exist_nodes
        self.train_clustering_coefficient = self.dataset.train_clustering_coefficient
        self.train_bidirectional_links_ratio = self.dataset.train_bidirectional_links_ratio

        self.test_right = self.dataset.test_right
        self.test_n_id = self.dataset.test_n_id
        self.test_edge_index = self.dataset.test_edge_index
        self.test_edge_type = self.dataset.test_edge_type
        self.test_exist_nodes = self.dataset.test_exist_nodes
        self.test_clustering_coefficient = self.dataset.test_clustering_coefficient
        self.test_bidirectional_links_ratio = self.dataset.test_bidirectional_links_ratio

        self.val_right = self.dataset.val_right
        self.val_n_id = self.dataset.val_n_id
        self.val_edge_index = self.dataset.val_edge_index
        self.val_edge_type = self.dataset.val_edge_type
        self.val_exist_nodes = self.dataset.val_exist_nodes
        self.val_clustering_coefficient = self.dataset.val_clustering_coefficient
        self.val_bidirectional_links_ratio = self.dataset.val_bidirectional_links_ratio

        
        if self.args.dataset_name == 'Twibot-20':
            padding_labels = 3 * torch.ones(229580 - len(self.labels), device=self.args.device).long()
            self.labels = torch.cat((self.labels, padding_labels), dim=0)

        self.args.window_size = self.dataset.window_size

        
        self.model = BotSTHCL(self.args)
        ...
        params = [
            ...
            {"params": self.model.contrastive_loss_weight, "lr": self.args.contrastive_learning_rate}  
        ]
    ...
    def forward_one_batch(self, batch_size, batch_n_id, batch_edge_index, batch_exist_nodes,
        ...
        edge_index_list = [edge.to(self.args.device) for edge in batch_edge_index]
        clustering_coefficient_list = [coeff.to(self.args.device) for coeff in batch_clustering_coefficient]
        bidirectional_links_ratio_list = [ratio.to(self.args.device) for ratio in batch_bidirectional_links_ratio]
        ...
        output, contrastive_loss = self.model(
        ...
        output = output.transpose(0, 1)  # [num_snapshots, batch_size, num_classes]

        
        loss = all_snapshots_loss(self.criterion, output, label_list, exist_nodes_list)

        
        return output, loss, label_list, exist_nodes_list, contrastive_loss
    ...
    def forward_one_epoch(self, right, n_id, edge_index, exist_nodes, clustering_coefficient, bidirectional_links_ratio):
        ...
        for batch_size, batch_n_id, batch_edge_index, batch_exist_nodes, batch_clustering_coefficient, batch_bidirectional_links_ratio \
            ...
            output, loss, label_list, exist_nodes_list, _ = self.forward_one_batch(
    ...
    def train_per_epoch(self, current_epoch):
        ...
        for batch_idx, (batch_size, batch_n_id, batch_edge_index, batch_exist_nodes, 
                       batch_clustering_coefficient, batch_bidirectional_links_ratio) in enumerate(
                       tqdm(zip(
                           self.train_right, 
                           self.train_n_id, 
            ...
            output, loss, label_list, exist_nodes_list, contrastive_loss = self.forward_one_batch(
                batch_size, batch_n_id, batch_edge_index, batch_exist_nodes,
                batch_clustering_coefficient, batch_bidirectional_links_ratio
            )

            
            contrastive_loss_weight = torch.nn.functional.softplus(self.model.contrastive_loss_weight)
            ...
            total_contrastive_loss += weighted_contrastive_loss.item() / self.args.window_size / len(self.train_right)

            
            (loss + weighted_contrastive_loss).backward()
            self.optimizer.step()
            ...
            if batch_idx % 100 == 0:
                ...
                print(f"Contrastive Loss Weight: {contrastive_loss_weight.item()}")
                if self.model.contrastive_loss_weight.grad is not None:
                    print(f"Gradient: {self.model.contrastive_loss_weight.grad.item()}")
                else:
                    print("Gradient: None")
        ...
        plog = f'Epoch-{current_epoch} train loss: {total_loss:.6f}, contrastive loss: {total_contrastive_loss:.6f}, weighted contrastive loss: {contrastive_loss_weight.item():.6f}' + plog
        print(plog)

       
        metrics['loss'] = total_loss
        metrics['contrastive_loss'] = total_contrastive_loss
        metrics['weighted_contrastive_loss'] = contrastive_loss_weight.item()
    ...
    def val_per_epoch(self, current_epoch):
        ...
        for key in ['accuracy', 'precision', 'recall', 'f1']:
            plog += f' {key}: {metrics[key]:.6f}'
        plog = f'Epoch-{current_epoch} val loss: {metrics["loss"]:.6f}' + plog
        print(plog)
        return metrics
    ...
    def test_last_model(self):
        ...
        for key in ['accuracy', 'precision', 'recall', 'f1']:
            plog += f' {key}: {metrics[key]:.6f}'
        plog = f'Last Epoch test loss: {metrics["loss"]:.6f}' + plog
        print(plog)
    ...
    def test_best_model(self, top_k=1):
        ...
        for epoch, state_dict in zip(self.test_epoch_list, self.test_state_dict_list):
            ...
            for key in ['accuracy', 'precision', 'recall', 'f1']:
                plog += f' {key}: {metrics[key]:.6f}'
            plog = f'Epoch-{epoch} test loss: {metrics["loss"]:.6f}' + plog
            print(plog)
    ...
    def train(self):
        ...
        for current_epoch in self.pbar:  
            ...
            if self.args.early_stop and validate_score_non_improvement_count >= self.args.patience:
                print(f'early stop in epoch: {current_epoch}')
                break  
        ...
        model_name = f"{self.args.interval}_{self.args.seed}_{self.test_metrics['accuracy']:.4f}_{self.test_metrics['precision']:.4f}_{self.test_metrics['f1']:.4f}.pt"
